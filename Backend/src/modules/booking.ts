...
bookingRouter.put("/:bookingId/checkin", async (req, res) => {
  try {
    ...
    try {
      const url = `https://smartdorm-detail.biwbong.shop/booking/${updated.bookingId}`;
      ...
      await sendFlexMessage(
        ...
        [{ label: "ดูรายละเอียด", url, style: "primary" }]
      );
    } catch (err) {
